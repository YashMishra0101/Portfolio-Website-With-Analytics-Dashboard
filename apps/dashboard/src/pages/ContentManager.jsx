...
export default function ContentManager() {
  ...
  return (
    <div className="max-w-7xl mx-auto w-full space-y-5 pb-10">
      ...
      ══════════════════════════════════════════════════════════════ */}
        <SectionCard>
          <SectionHeader icon={<Image size={13} />} label="Profile Photo" color="emerald" badge="image url" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
            ...
            <div className="space-y-4">
              <Field
                label="Photo URL"
                hint="Paste any valid image URL here"
              >
                <input
                  type="url"
                  readOnly={isReadOnly}
                  className="tactical-input"
                  value={content.profilePhotoUrl || ""}
                  onChange={(e) => updateField("profilePhotoUrl", e.target.value)}
                  placeholder="https://..."
                />
              </Field>

              <div className="p-3 bg-zinc-900/60 border border-zinc-800/60 rounded-sm space-y-1.5">
                <p className="text-[9px] text-emerald-500 font-mono uppercase tracking-widest font-bold">Secure Approach</p>
                <p className="text-[9px] text-zinc-500 font-mono leading-relaxed mt-1">
                  To guarantee 100% security, the dashboard does not handle direct file uploads.
                  Simply host your photo somewhere (like Imgur, GitHub, or Cloudinary) and paste the direct link above.
                </p>
              </div>

              {!content.profilePhotoUrl?.trim() && (
                <div className="flex items-center gap-2 p-2.5 bg-amber-950/20 border border-amber-900/40 rounded-sm">
                  <ImageOff size={12} className="text-amber-500 shrink-0" />
                  <p className="text-[9px] text-amber-500/80 font-mono">
                    No URL set — portfolio is showing a blank avatar placeholder.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="w-32 h-32 rounded-full overflow-hidden border-2 border-zinc-700 bg-zinc-900 flex items-center justify-center">
                {content.profilePhotoUrl?.trim() ? (
                  <img
                ...
                <div
                  ...
                  style={{ display: content.profilePhotoUrl?.trim() ? "none" : "flex" }}
                >
                  <Image size={28} className="text-zinc-700" />
                </div>
              </div>
              {content.profilePhotoUrl?.trim() && (
                <p className="text-[9px] text-emerald-500 font-mono">✓ URL set — portfolio will use this photo</p>
              )}
            </div>
          </div>
        </SectionCard>
